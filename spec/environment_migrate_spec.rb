...
describe KnifeMigrate::EnvironmentMigrate do
  ...
  context '#update_attrs' do
    ...
    it 'returns updated attributes' do
      ...
      expect(ui_obj).to receive(:ask_question).and_return('debug')
      plugin.update_attrs
    ...
    # it 'sets up missing default attributes' do
      # expect(plugin).to receive(:missing_attrs).and_return(
         # [{
           ...
           # }}]
      # )
      # expect(plugin).to receive(:ui).and_return(ui_object).exactly(10).times
      # expect(ui_object).to receive(:msg).exactly(3).times
